package com.lirctek.loadboard.ui.payments.notBilled
...
@Composable
fun NotBilledUi(navController: NavController){
    LaunchedEffect(Unit){ isFirstTimeShimmer = true }

    val viewModel = hiltViewModel<NotBilledViewModel>()
    ...
    if (status != ConnectivityObserver.Status.Available && state.loadsList.isEmpty()){
        ...
        SwipeRefresh(
            ...
            Column(modifier = Modifier.fillMaxSize()) {
                ...
                PaymentsBilledAndInvoicedUi(type = PaymentsConstants.NOT_BILLED)
                PaymentsBilledAndInvoicedUi(type = PaymentsConstants.NOT_BILLED)
                PaymentsBilledAndInvoicedUi(type = PaymentsConstants.NOT_BILLED)
                PaymentsBilledAndInvoicedUi(type = PaymentsConstants.NOT_BILLED)
            }
